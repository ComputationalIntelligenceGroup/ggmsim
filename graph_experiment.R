...
## port_chol
Adj <- igraph::as_adjacency_matrix(ug, sparse = FALSE)

system.time(sample <- gmat::port_chol(N, ug = ug, h = 10000, eps = 0.5))
...
## port
system.time(sample <- gmat::port(N, ug = ug))
...
### diagdom
system.time(sample <- gmat::diagdom(N, ug = ug))
...
Adjch <- gRbase::triangulate(Adj)
colnames(Adjch) <- rownames(Adjch) <- NULL

ugch <- igraph::graph_from_adjacency_matrix(Adjch, mode = "undirected")
plot(ugch)


## port
system.time(sample <- gmat::port(N, ug = ugch))
reduced <- as.data.frame(t(apply(sample, 3, function(mat) {
...
## port_chol
system.time(sample <- gmat::port_chol(N, ug = ugch))
reduced <- as.data.frame(t(apply(sample, 3, function(mat) {
  return(mat[upper.tri(Adjch) & Adjch == 1])
})))
ggplot(stack(reduced)) + geom_density(aes(x = values, group = ind)) +
...
### diagdom
system.time(sample <- gmat::diagdom(N, ug = ugch))
sample <- array(dim = dim(sample), data = apply(sample, 3, cov2cor))
reduced <- as.data.frame(t(apply(sample, 3, function(mat) {
  return(mat[upper.tri(Adjch) & Adjch == 1])
})))
