...
plot_map_reduce <- function(p, d, N, map = function(x) { return(x)},
	...
	res <- matrix(nrow = length(p), ncol = d_len,
																dimnames = list(p = p, d = d))
	res_sd <- matrix(nrow = length(p), ncol = d_len,
								dimnames = list(p = p, d = d))
...
plot_map_reduce_cmp <- function(p, d, N, map = function(x) {return(x)},
	...
	res <- matrix(nrow = length(p), ncol = 2,
								dimnames = list(p = p, method = methods))
	res_sd <- matrix(nrow = length(p), ncol = 2,
									 dimnames = list(p = p, method = methods))
...
plot_eigen_spec <- function(p, d, method, fname, dir_name = "res",
	...
	eigen_vals <- apply(sample, MARGIN = 3, function(m) { return(eigen(m)$values)})
...
plot_eigen_freqpol <- function(p, d, method, fname, dir_name = "res", bin_fun = nclass.Sturges,
															 sample_size = 100, plot_title = "", ...) {
	d_len <- length(d)
	exp_fname <- paste0(p, "_", d, ".rds")

	eigen_vals <- array(dim = c(d_len, p*sample_size), dimnames = list(d = d))
	for (i in 1:d_len) {
		sample <- readRDS(file = paste0(dir_name,"/", method, "_", exp_fname[i]))
		eigen_vals[i,] <- apply(sample, MARGIN = 3, function(m) { return(eigen(m)$values)})
	}
	wd <- getwd()
	dir.create(paste0(wd, "/plot_",dir_name), showWarnings = FALSE)
